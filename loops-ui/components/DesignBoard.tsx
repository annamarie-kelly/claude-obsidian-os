...
export function DesignBoard({
  ...
  onFix?: (imgPath: string, annotations: { index: number; comment: string }[]) => void;
}) {
  ...
  const renderAndSave = useCallback(async (): Promise<{ imgPath: string; annSummary: { index: number; comment: string }[] } | null> => {
    ...
    return { imgPath: imgFile, annSummary };
  }, [board, addSavedBoard]);
  ...
  const handleFix = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => setExported(false), 3000);
      if (result && onFix) {
        onFix(result.imgPath, result.annSummary);
      }
    } catch (err) {
